...
type NavItem = { path: string; label: string; icon: Icon; group: 'overview' | 'care' | 'operations' | 'administration' | 'support'; permission?: string };

const navItems: NavItem[] = [
  { path: '/', label: 'Dashboard', icon: HomeIcon, group: 'overview' },
  { path: '/patients', label: 'Patients', icon: UsersIcon, group: 'care', permission: 'patient.read' },
  { path: '/admissions', label: 'Admissions', icon: UserPlusIcon, group: 'care', permission: 'clinical.read' },
  { path: '/clinical-notes', label: 'Clinical Notes', icon: ClipboardDocumentListIcon, group: 'care', permission: 'clinical.read' },
  { path: '/vitals', label: 'Vitals', icon: HeartIcon, group: 'care', permission: 'vitals.read' },
  { path: '/medications', label: 'Medication', icon: CubeIcon, group: 'care', permission: 'clinical.read' },
  { path: '/appointments', label: 'Appointments', icon: CalendarDaysIcon, group: 'operations', permission: 'appointment.read' },
  { path: '/billing', label: 'Billing', icon: CreditCardIcon, group: 'operations', permission: 'billing.read' },
  { path: '/inventory', label: 'Inventory', icon: CubeIcon, group: 'operations', permission: 'inventory.read' },
  { path: '/reports', label: 'Reports', icon: ChartBarIcon, group: 'operations' },
...
function Navigation({ collapsed, onNavigate }: { collapsed: boolean; onNavigate: () => void }) {
  const { isSuperAdmin, isPlatformAdmin, isRehabAdmin, permissions } = useAuth(); const { pathname } = useLocation();
  ...
  const permitted = (item: NavItem) => !item.permission || isSuperAdmin || permissions.includes(item.permission) || (item.path === '/settings' && isRehabAdmin);
...
export default function Sidebar() {
  const [collapsed, setCollapsed] = useState(false); const [mobileOpen, setMobileOpen] = useState(false); const { pathname } = useLocation(); const { isPlatformAdmin } = useAuth();
  const mobileItems = isPlatformAdmin ? [{ path: '/admin', label: 'Admin', icon: BuildingOffice2Icon, group: 'overview' as const }, { path: '/admin/plans', label: 'Plans', icon: CreditCardIcon, group: 'administration' as const }] : navItems.filter((item) => ['/', '/patients', '/appointments', '/billing'].includes(item.path));
