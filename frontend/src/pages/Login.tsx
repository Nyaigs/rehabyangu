...
const Login: React.FC = () => {
  ...
  return <main className="min-h-screen bg-[#e9efec] p-4 text-slate-900 sm:p-6 lg:p-8">
    <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-[#fbfcfa] shadow-[0_8px_28px_rgba(15,46,61,.06)] sm:min-h-[calc(100vh-3rem)] lg:grid-cols-[1.02fr_.98fr]">
      ...
      <section className="flex items-center justify-center px-6 py-12 sm:px-12 lg:px-16">
        <div className="w-full max-w-[390px] animate-fade-in"><div className="mb-10 lg:hidden"><Brand branding={branding} dark /></div><div className="mb-8"><p className="text-sm font-semibold text-[#17614f]">Welcome back</p><h2 className="mt-2 text-[30px] font-bold tracking-tight text-[#102c3c]">Sign in to your workspace</h2><p className="mt-3 text-[15px] leading-6 text-slate-600">Use your facility workspace and account details to continue.</p></div>
          <form onSubmit={handleSubmit} className="space-y-5">
            <Field label="Workspace" icon={<BuildingOffice2Icon className="h-4 w-4" />}><input value={tenantSlug} onChange={(event) => setTenantSlug(event.target.value)} required placeholder="Your workspace" className="h-12 w-full rounded-xl border border-slate-300 bg-white pl-11 pr-3 text-[15px] font-medium text-slate-900 outline-none transition placeholder:font-normal placeholder:text-slate-400 focus:border-[#17614f] focus:ring-4 focus:ring-emerald-50" /><span className="mt-1.5 block text-xs font-medium text-slate-600">This is your facility's short name.</span></Field>
          ...
          </form><p className="mt-5 text-center text-sm font-medium text-slate-700">Forgot your password? Contact your facility administrator, or <a className="font-semibold text-[#17614f] underline hover:text-[#104c3e]" href="mailto:admin@rehabyangu.com?subject=RehabYangu%20access%20help">email RehabYangu support</a>.</p><p className="mt-4 text-center text-sm leading-5 text-slate-600">By continuing, you confirm that you are authorised to access this clinical workspace.</p>
        </div>
      </section>
    </div>
  </main>;
...
function Field({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) { return <label className="block"><span className="mb-2 block text-sm font-semibold text-slate-700">{label}</span><span className="relative block"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-900 [&>svg]:stroke-[2.25]">{icon}</span>{children}</span></label>; }
